...
def read_odi_data():
    """Reads ODI JSON files and returns structured DataFrames."""
    # Data containers
    match_info = []
    innings_data = []
    over_data = []
    ...
    # Look for ODI JSON files
    json_files = [f for f in os.listdir(data_folder) if f.endswith('.json') and 'odi' in f.lower()]
    
    if not json_files:
        print("No ODI data JSON files found.")
        return {}
    
    print(f"Processing {len(json_files)} ODI data files")
